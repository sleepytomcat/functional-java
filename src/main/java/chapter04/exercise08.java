...
public class exercise08 {
	public static void main(String... args) {
		...
		for (int i=0; i<10000; i++) functions.add(x->x+1);

		Function<Integer, Integer> combined = composeAll(functions);
		System.out.println(combined.apply(9));
	}

	public static <T> Function<T, T> composeAll(List<Function<T,T>> functions) {
		return x -> foldLeftStackSafe(x, reverseWithFoldLeft(functions), acc -> f -> f.apply(acc)).eval();
	}
	...
	public static <T> List<T> reverseWithFoldLeft(List<T> list) {
		return exercise08.<T,List<T>>foldLeftStackSafe(list(), list, x -> y -> prepend(y, x)).eval();
	}
