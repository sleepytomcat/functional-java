...
public class exercise06 {
	...
	public static <T> Function<T, T> composeAll(List<Function<T,T>> functions) {
		Function<Function<T,T>, Function<Function<T,T>, Function<T,T>>> folding = acc -> f -> x -> f.apply(acc.apply(x));
		Function<T,T> result = foldLeftStackSafe(m -> m, functions, folding).eval();
		return result;
	}
